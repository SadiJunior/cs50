...
// Buffer to read dog names into11
char name[MAXCHAR];
...
bool check(char *word)
{
    // Initializes the cursor in the root node
    node *cursor = root;
    ...
    for (int i = 0; i < str_len; i++)
    {
        ...
        // If the letter isn't contained in the trie
        if (cursor->children[letter_index] == NULL)
        {
            return false;
        }

        // If it got to the last letter
        if (i == str_len - 1)
        {
            // If the node letter is also the last letter
            if (cursor->children[letter_index]->is_word)
            {
                return true;
            }
        }
